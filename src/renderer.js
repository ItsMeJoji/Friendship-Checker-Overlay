...
export function update() {
    state.pokemon.forEach((p, index) => {
        ...
        // Bounce edges
        const width = p.isDynamaxed ? p.img.width * 3 : p.img.width;
        const height = p.isDynamaxed ? p.img.height * 3 : p.img.height;
        ...
        for (let i = index + 1; i < state.pokemon.length; i++) {
            ...
            const dx = (p.x + width/2) - (p2.x + (p2.isDynamaxed ? p2.img.width*3 : p2.img.width)/2);
            const dy = (p.y + height/2) - (p2.y + (p2.isDynamaxed ? p2.img.height*3 : p2.img.height)/2);
            const distance = Math.hypot(dx, dy);
            const minSourceDist = (width + (p2.isDynamaxed ? p2.img.width * 3 : p2.img.width)) / 4;
...
export function draw() {
    ...
    state.pokemon.forEach(p => {
        ...
        if (p.message && !hasLink && Date.now() < p.messageTimer) {
            ...
            state.ctx.strokeText(p.message, textX, p.y - 10);
            state.ctx.fillText(p.message, textX, p.y - 10);
        }
    });
}
